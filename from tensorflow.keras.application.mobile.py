...
weightsPath = r"face.detector\res10_300x300_ssd_iter_140000.caffcoodel"
...
while True:
    ...
    for(box , pred) in zip(locs,preds):
        ...
        label="Mask" if ask > withoutMask else "No Mask"
        color = (0,225,0) if label =="Mask " else(0,0,225)
        
        label ="{}: {:2f}%" ,format(label,max(mask,withoutMask)*100)
        
        cv2.putText(frame,label,(startX ,startY -10),
                    ev2.FONT_HERSHEY_SIMPLEX,0.45,color,2)
